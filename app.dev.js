...
document.addEventListener('DOMContentLoaded', () => {
  ...
  (function initGyroscopeParallax() {
    ...
    // Permission button handling for iOS 13+
    const promptBtn = document.getElementById('gyro-permission-prompt');

    function dismissPrompt() {
      if (!promptBtn) return;
      promptBtn.classList.add('is-hidden');
      setTimeout(() => {
        if (promptBtn.parentNode) promptBtn.parentNode.removeChild(promptBtn);
      }, 300);
    }
    ...
    function requestPermission() {
      if (typeof DeviceOrientationEvent !== 'undefined' && typeof DeviceOrientationEvent.requestPermission === 'function') {
        DeviceOrientationEvent.requestPermission()
          ...
          .catch(() => {
            // Keep prompt visible so user can tap again if denied or dismissed
          });
      } else {
        attachOrientationListener();
        dismissPrompt();
    ...
    function cleanupGestures() {
      ...
      window.removeEventListener('touchend', gestureTrigger);
    }
    ...
    function gestureTrigger(e) {
      ...
      // If user tapped the permission prompt or any gyro card
      if (e.target.closest('#gyro-permission-prompt') || e.target.closest('.gyro-card')) {
        requestPermission();
      }
    }
    ...
    if (requiresPermission) {
      // Show permission button on iOS touch devices
      if (promptBtn) {
      ...
      // Also allow tapping directly on any gyro-card
      window.addEventListener('click', gestureTrigger, { passive: true });
      window.addEventListener('touchend', gestureTrigger, { passive: true });
    } else if ('ondeviceorientation' in window || 'DeviceOrientationEvent' in window) {
      // Android / non-iOS touch devices: auto-bind immediately
      attachOrientationListener();
    }
